...
export default class HallwayScene {
  constructor(
    ...
    this.hud =
      existingPlayer && existingPlayer.hud ? existingPlayer.hud : new HUD();
    ...
    if (existingPlayer) {
      ...
      console.log(
        `✅ Player entered hallway puzzle with ${this.player.health.current} HP`
      );
    } else {
  ...
  createFloor() {
    ...
    const tryLoadTexture = (index) => {
      ...
      textureLoader.load(
        ...
        (texture) => {
          console.log(
            "✅ Floor tile texture loaded from:",
            texturePaths[index]
          );
        ...
        (error) => {
          console.log(
            "❌ Failed to load floor texture from:",
            texturePaths[index]
          );
  ...
  createCeiling() {
    ...
    const tryLoadTexture = (index) => {
      ...
      textureLoader.load(
        ...
        (error) => {
          console.log(
            "❌ Failed to load ceiling texture from:",
            texturePaths[index]
          );
          tryLoadTexture(index + 1);
        }
  ...
  addWindowsAndArtwork() {
    ...
    artworkPositions.forEach((pos, index) => {
      ...
      const artColors = [
        0x4a2a1a, 0x2a3a4a, 0x3a1a2a, 0x1a2a1a, 0x4a3a2a, 0x2a2a3a,
      ];
  ...
  addAtmosphericDetails() {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      chain.position.set((Math.random() - 0.5) * 8, 5, Math.random() * 30 - 15);
      ...
      if (index % 3 === 0) {
        ...
        const animateRune = () => {
          rune.material.opacity =
            0.2 + Math.sin(Date.now() * 0.002 + index) * 0.2;
  ...
  createTilePuzzle() {
    ...
    console.log(
      "Safe tiles pattern:",
      safeTiles.map((s) => (s === 0 ? "LEFT" : "RIGHT"))
    );
    ...
    for (let i = 0; i < this.totalTilePairs; i++) {
      ...
      const leftTile = this.createTile(
        -2.5,
        pairZ,
        tileWidth,
        tileDepth,
      ...
      const rightTile = this.createTile(
        2.5,
        pairZ,
        tileWidth,
        tileDepth,
  ...
  animateLighting() {
    ...
    this.ceilingLights?.forEach((light, index) => {
      const flicker =
        Math.sin(time * light.userData.flickerSpeed + index) * 0.1;
      light.intensity = light.userData.baseIntensity + flicker;
    });

    if (this.puzzleComplete && this.doorLight) {
      this.doorLight.intensity =
        this.doorLight.userData.baseIntensity + Math.sin(time * 2) * 0.5;
    }
  }
  ...
  selectTile(side) {
    ...
    this.hud.showMessage(
      ...
      } tile selected - Press SPACE to move`
    );
  }
    ...
    this.tiles.forEach((tile) => {
      if (
        ...
        if (tile.userData.isSelected) {
          ...
          const edgeLines = tile.children.find(
            (child) => child.type === "LineSegments"
          );
          if (edgeLines) {
            edgeLines.material.opacity =
  ...
  moveToSelectedTile() {
    ...
    const selectedTile = this.tiles.find(
      (tile) =>
        tile.userData.pairIndex === this.currentTilePair &&
    ...
    if (this.player.ghost) {
      ...
      const animateMove = () => {
        ...
        if (progress > 0.8) {
          ...
          this.player.ghost.position.y +=
            Math.sin(bounceProgress * Math.PI) * 0.3;
        }
  ...
  stepOnTile(tile) {
    if (!tile.userData.activated) {
      ...
      if (tile.userData.isSafe) {
        ...
        this.hud.showMessage(
          `✅ SAFE! Progress: ${this.currentTilePair + 1}/${
            this.totalTilePairs
          }`
        );
  ...
  flashScreen(color) {
    ...
    flash.style.background = `rgba(${(color >> 16) & 255}, ${
      (color >> 8) & 255
    }, ${color & 255}, 0.6)`;
  ...
  respawnPlayer() {
    ...
    if (this.currentTilePair > 0) {
      ...
      const lastSafeTile = this.tiles.find(
        (tile) =>
    ...
    this.hud.showMessage(
      `⚠️ Damaged! ${this.player.health.current} HP remaining - Continue from here!`
    );
    setTimeout(() => {
  ...
  handlePlayerDeath() {
    this.hud.showMessage(
      "💀 You ran out of health! Restarting from the beginning..."
    );
  ...
  enterBathroom() {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // CRITICAL: Clear the message AND remove overlay
        this.hud.showMessage(""); // ← Clears "Entering the bathroom..."
  ...
  showPuzzleInstructions() {
    ...
    const startPuzzle = (e) => {
      if (e.code === "Space") {
        ...
        this.hud.showMessage(
          "Choose your first tile using LEFT ◀️ or RIGHT ▶️ arrow"
        );
